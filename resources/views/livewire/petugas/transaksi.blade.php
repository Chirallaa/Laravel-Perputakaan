<div class="row">
    ...
    <div class="btn-group mb-3">
        ...
        <button wire:click="belumDipinjam" class="btn btn-sm bg-indigo mr-2">Tersedia</button>
                ...
                @foreach ($transaksi as $item)
                    <tr>
                        ...
                        <td>
                                ...
                                <span>{{$detail_peminjaman->buku->judul}}</span>
                                @endforeach
                        </td>
                       <td>
                                @foreach ($item->detail_peminjaman as $detail_peminjaman)
                                <span>{{$detail_peminjaman->buku->rak->lokasi}}</span>
                                @endforeach
                        </td>
                        <td>
                            @if ($item->status == 1)
                       ...
                       @if (!$selesai_dipinjam)
                            <td>
                                <span wire:click="show({{$item->id}})" class="btn btn-sm btn-info mr-2">Detail</span>
                                @if ($item->status == 1)
                                ...
                                @elseif ($item->status == 3)
                                    <span wire:click="delete({{$item->id}})" class="btn btn-sm btn-danger mr-2">Hapus</span>
                                @endif
                            </td>
                       @endif
                    </tr>
